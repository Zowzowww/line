...
from PoseModule import PoseDetector  # Ensure predict_pose is imported

app = Flask(__name__)

# Initialize video capture and pose detector
cap = cv2.VideoCapture(0)  # 0 for one camera; use 1 for dual cameras
...
def generate_frames():
    ...
    while True:
        ...
        if not success:
            ...
            lr_result, nb_result = detector.predict_pose()
            lmList = detector.find_position(img, False)

            # Only proceed if landmarks are detected
            if len(lmList) != 0:
                # Use predict_pose to classify the current pose
            
               

                # Display predictions on the frame
                cv2.putText(img, f'LR: {lr_result}', (600, 50), cv2.FONT_HERSHEY_PLAIN, 3, (255, 0, 0), 3)
                cv2.putText(img, f'NB: {nb_result}', (600, 100), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0), 3)
